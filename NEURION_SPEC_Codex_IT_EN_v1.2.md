# NEURION — Software Specification & Build Plan v1.2
...
# 0.0 v1.2 Changelog — Gap Resolutions

v1.2 integra la risoluzione di 15 gap trovati nell'analisi della spec v1.1. Le decisioni di design complete (codice + Prisma + spec patch) vivono in `docs/architecture/`. Sintesi:

```txt
G1  Verifica compute    docs/architecture/verification-g1.md
    Optimistic delivery + verifica async a livelli (sanity sempre + deep re-exec/golden
    campionato) + reputation EWMA + verificatore SOLO trusted (chiude collusione).
    Reward decoupled da delivery: COMPLETED->VERIFIED->REWARDED. Probation 30 job,
    retro-audit + SUSPEND + clawback su fail. NRN payout solo post-VERIFIED.

G2  Privacy prompt       docs/architecture/privacy-g2.md
    ChatConversation.privacyLevel default = VERIFIED_ONLY. Nodo COMMUNITY non vede mai
    chat plaintext by default. Hard trust-set filter (COMMUNITY ammesso sse e solo se
    effective==PUBLIC + consenso open-tier vivo). Classifier advisory + fail-safe-up.
    TEE attestation gate. Response buffer-until-verified per community.

G3  Chat reale           MVP chat usa modello REALE via Fallback (ollama). Mock = solo test,
    sempre etichettato. AI_PROVIDER_DEFAULT non e' piu' mock di default.

G4  Scope job            MVP = echo.v1 + embedding.v1. Esempio "40 PDF" = embedding+RAG+chat.
    Audio/OCR/image = JOB_TYPES registry-disabled "Coming soon" (mai bottoni finti).

G5-G15  Gap residui      docs/architecture/gaps-g5-g15.md
    G5 sybil (owner=unita' di trust, stake registrazione, cap per-owner, KYC gate payout)
    G6 RefreshToken model + rotation + reuse-detection
    G7 Attachment model + MinIO presigned + retention (eredita privacy G2)
    G8 estimator pre-pass cheap (bytes + char/4 + heavy-set)
    G9 ChatMessage.linkedJob relation
    G10 stima display-only; HOLD pre-auth -> SETTLE su uso reale
    G11 WS scaling: MVP single-process; path = Redis pub/sub EventBus
    G12 EmissionSchedule: budget/epoch da curva 8y, HALT su pool esaurito
    G13 TOKEN_PAYOUTS_ENABLED in env + guard
    G14 signer custody: testnet-only; prod = Gnosis Safe multisig
    G15 worker isolation hardening (docker --cap-drop ALL, --read-only, --pids-limit, ...)
```

Le sezioni 8/9/13/26 sotto sono aggiornate per riflettere queste decisioni; il dettaglio implementativo (modelli Prisma nuovi, servizi, codice) e' nei doc `docs/architecture/`.
...
AI_PROVIDER_DEFAULT=openai_compatible   # G3: real ollama in dev; 'mock' is test-only + always labeled
...
# Feature flags (G13)
TOKEN_PAYOUTS_ENABLED=true          # global kill-switch for NRN payouts; true only for local/testnet
REGISTRATION_STAKE_CREDITS=50       # G5 refundable node-registration stake
KYC_PAYOUT_THRESHOLD_CREDITS=1000   # G5 NRN payout above this requires KYC_APPROVED
EMISSION_EPOCH_DAYS=1               # G12 reward-emission accounting epoch
...
> **v1.2 note:** the entities below are the v1.1 baseline. v1.2 adds the gap models — `JobVerification`, `GoldenTask`, `NodeVerification` fields (G1); `NodeConfidentialityProfile`, `NodeAttestation`, `NodeOperatorAgreement(+Acceptance)`, `ChatMessageRoute`, `OpenTierConsent` (G2); `OwnerReputation`, `RegistrationChallenge`, `WorkspaceConfig`, `NodeLifecycle` enum, `CreditLedger.idempotencyKey` (G5); `RefreshToken` (G6); `Attachment` (G7); `ChatMessage.linkedJob` relation (G9); `EmissionSchedule` (G12). Full Prisma deltas in `docs/architecture/{verification-g1,privacy-g2,gaps-g5-g15}.md`. The consolidated schema lives in `apps/api/prisma/schema.prisma`.
...
# 26. Definition of Done — MVP v1.2

MVP v1.2 is done when:
...
- chat streams response using a REAL model via Fallback lane (ollama llama3.1:8b/phi3:mini)
  [G3: mock provider is test-only and always renders a visible "[MOCK]" banner]
