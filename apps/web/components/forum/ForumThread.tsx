...
interface Thread {
  ...
  pinned: boolean; locked: boolean; createdAt: string; author: Author; posts: Post[];
}

export function ForumThread({ canPost, backHref, loginHref }: { canPost: boolean; backHref: string; loginHref: string }) {
  const t = useT();
  const { user } = useAuth();
  ...
  const load = () => api<Thread>(`/forum/threads/${tid}`).then(setTh).catch(() => setTh(null));
  ...
  async function sendReply(e: React.FormEvent) {
    ...
    setBusy(true);
    try { await api(`/forum/threads/${tid}/posts`, { method: 'POST', body: JSON.stringify({ body: reply }) }); setReply(''); await load(); }
    finally { setBusy(false); }
  }
  const delPost = async (pid: string) => { await api(`/forum/posts/${pid}`, { method: 'DELETE' }).catch(() => undefined); await load(); };
  const delThread = async () => { await api(`/forum/threads/${tid}`, { method: 'DELETE' }).catch(() => undefined); router.push(backHref); };
  const moderate = async (d: { pinned?: boolean; locked?: boolean }) => { await api(`/forum/threads/${tid}`, { method: 'PATCH', body: JSON.stringify(d) }).catch(() => undefined); await load(); };
  ...
  return (
    <div style={{ maxWidth: 760 }}>
      <div style={{ fontSize: 13, color: theme.muted, marginBottom: 8 }}>
        <Link href={backHref} style={{ color: theme.accent, textDecoration: 'none' }}>{t('forum.title')}</Link>
        {' · '}{th.section.icon} {th.section.name}
      </div>
      <h1 style={{ margin: '0 0 12px', fontSize: 22 }}>{th.pinned && '📌 '}{th.locked && '🔒 '}{th.title}</h1>
      ...
      {(canDelThread || (canPost && isMod)) && (
        <div style={{ display: 'flex', gap: 8, margin: '10px 0' }}>
          {canPost && isMod && <button onClick={() => void moderate({ pinned: !th.pinned })} style={ghost}>{th.pinned ? t('forum.unpin') : t('forum.pin')}</button>}
          {canPost && isMod && <button onClick={() => void moderate({ locked: !th.locked })} style={ghost}>{th.locked ? t('forum.unlock') : t('forum.lock')}</button>}
          {canDelThread && <button onClick={() => void delThread()} style={{ ...ghost, color: '#e0533d', borderColor: '#e0533d' }}>{t('forum.deleteThread')}</button>}
        </div>
      )}

      <h3 style={{ fontSize: 14, color: theme.muted, textTransform: 'uppercase', letterSpacing: '.08em', margin: '22px 0 8px' }}>{th.posts.length} {t('forum.replies')}</h3>
      {th.posts.map((p) => (
      ...
      {canPost ? (
        ...
        <div style={{ marginTop: 16, padding: 14, border: `1px dashed ${theme.border}`, borderRadius: 12, textAlign: 'center' }}>
          ...
          <Link href={loginHref} style={{ color: theme.accent, fontSize: 13, textDecoration: 'none' }}>{t('forum.participate')} →</Link>
        </div>
      )}
    </div>
  );
