...
export function SessionsSidebar() {
  ...
  const load = () => {
    void api<any[]>("/chat/conversations")
      .then(setConversations)
      .catch(() => undefined);
    void api<any[]>("/projects")
      .then(setProjects)
      .catch(() => undefined);
  };
  useEffect(() => {
  ...
  async function delConv(id: string) {
    await api(`/chat/conversations/${id}`, { method: "DELETE" }).catch(
      () => undefined,
    );
  ...
  function onDrop(e: React.DragEvent) {
    ...
    if (f?.path) {
      void createProjectWithPath(
        f.name || t("sessions.defaultProjectName"),
  ...
  const Item = (c: any) => (
    <div
      ...
      style={{
        ...
        borderRadius: 8,
        cursor: "pointer",
        background: activeId === c.id ? theme.surface : "transparent",
      }}
    >
      <span
        style={{
          ...
          whiteSpace: "nowrap",
          fontSize: 13,
          color: activeId === c.id ? theme.text : theme.muted,
        }}
      >
      ...
      <span
        onClick={(e) => {
          ...
          const sel = window.prompt(
            t("sessions.promptAssignToProject") + "\n" + list,
          );
          if (sel === null) return;
      ...
      <span
        onClick={(e) => {
          e.stopPropagation();
          void pinConv(c.id, !c.pinned);
        }}
        title={t("sessions.pinTooltip")}
        style={{ color: c.pinned ? theme.accent : theme.muted, fontSize: 12 }}
      >
        {c.pinned ? "★" : "☆"}
      </span>
      <span
  ...
  return (
    <div
      ...
      style={{
        ...
        paddingRight: 4,
        borderRadius: 8,
        background: dragOver ? theme.surface : "transparent",
      }}
    >
      <button
        onClick={newSession}
        style={{ ...ghostButton, textAlign: "left", marginBottom: 8 }}
      >
        ...
        return (
          <div key={p.id}>
            ...
            >
              ...
              <span
                onClick={async () => {
                  const conv = await api<any>("/chat/conversations", {
                    method: "POST",
                    body: JSON.stringify({ title: t("sessions.newChatTitle") }),
                  });
                  await api(`/chat/conversations/${conv.id}`, {
                    method: "PATCH",
