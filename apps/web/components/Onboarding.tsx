...
export function Onboarding() {
  ...
  const [rec, setRec] = useState<{
    ramGb: number;
    cores: number;
    model: { name: string; label: string; size: string; note: string };
  } | null>(null);
  ...
  async function download() {
    ...
    const [path, body] =
      mode === "bundled"
        ...
        : ([
            "/ai/models/pull",
            { name: rec?.model.name ?? RECOMMENDED_MODEL },
          ] as const);
    try {
      await streamSSE(path, body, {
        onEvent: (event, d) => {
          if (event === "progress") {
            ...
            setErr(d.message || "download failed");
            setPull(null);
          }
        },
      });
      ...
      <div style={panel}>
        {step === "welcome" && (
          <>
            ...
            >
              <button
                style={{ ...button, padding: "9px 22px" }}
                onClick={() => setStep(engineUp ? "model" : "engine")}
              >
                {t("ob.next")} →
              </button>
              {skip}
        ...
        {step === "engine" && (
          <>
            ...
            >
              <span className="flicker" style={{ color: theme.amber }}>
                ●
              </span>{" "}
              {t("ob.engineWaiting")}
            </div>
        ...
        {step === "model" && (
          <>
            ...
            {mode === "bundled" && bundled && !pull && (
              ...
              >
                <div
                  style={{ fontSize: 12, color: theme.muted, marginBottom: 5 }}
                >
                  ⚡ {t("ob.autoRec")}
                </div>
                <div
            ...
            {mode === "ollama" && rec && !pull && (
              ...
              >
                <div
                  style={{ fontSize: 12, color: theme.muted, marginBottom: 5 }}
                >
                  🖥 {rec.ramGb} GB RAM · {rec.cores} core → {t("ob.autoRec")}
                </div>
                <div
                  style={{ fontSize: 15, fontWeight: 600, color: theme.text }}
                >
            ...
            {pull ? (
              <div style={{ marginBottom: 20 }}>
                ...
                >
                  <div
                    style={{
                      height: "100%",
                      width: `${Math.max(3, pull.percent ?? 3)}%`,
              ...
              <button
                style={{ ...button, padding: "10px 22px", marginBottom: 16 }}
            ...
            {err && (
              <div style={{ color: "#e0533d", fontSize: 12, marginBottom: 10 }}>
                ⚠ {err}
              </div>
            )}
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
