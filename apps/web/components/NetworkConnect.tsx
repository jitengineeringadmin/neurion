  ...
  return (
    ...
    >
      <div style={{ fontSize: 12, color: theme.muted, marginBottom: 8 }}>
        {t("network.connectPrompt")}{" "}
        <span style={{ color: theme.text }}>
          {PROD_BASE.replace("https://", "")}
        </span>
      </div>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <input
          ...
          style={{ ...input, width: 160, padding: "6px 8px" }}
        />
        <input
          value={pass}
        ...
        <button
          ...
          disabled={busy}
          style={{ ...button, padding: "6px 14px", opacity: busy ? 0.5 : 1 }}
        >
          {busy ? "…" : t("network.connect")}
        </button>
      </div>
      {err && (
        <div style={{ color: "#e0533d", fontSize: 12, marginTop: 6 }}>
          ⚠ {err}
        </div>
      )}
    </div>
