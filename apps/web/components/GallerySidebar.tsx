...
interface Item { id: string; status: string; prompt?: string; image?: string; createdAt?: string }
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, paddingRight: 4 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
        {items.map((it) => (
          it.status === 'done' && it.image ? (
            <a key={it.id} href={`data:image/png;base64,${it.image}`} download={`neurion-${it.id}.png`} title={it.prompt || ''}
              style={{ display: 'block', borderRadius: 8, overflow: 'hidden', border: `1px solid ${theme.border}`, lineHeight: 0 }}>
              <img src={`data:image/png;base64,${it.image}`} alt="" style={{ width: '100%', aspectRatio: '1', objectFit: 'cover' }} />
            </a>
          ) : (
