...
export function GallerySidebar() {
  ...
  useEffect(() => {
    let alive = true;
    const load = () =>
      void api<{ items: Item[] }>("/ai/image/gallery")
        .then((r) => {
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
        {items.map((it) =>
          it.status === "done" && it.hasImage ? (
            <div
              key={it.id}
              onClick={() => void downloadImage(it.id, `neurion-${it.id}.png`)}
              title={it.prompt || ""}
              style={{
                display: "block",
                borderRadius: 8,
            ...
            <div
              ...
              style={{
                ...
                fontSize: 11,
                color: it.status === "failed" ? "#e0533d" : theme.amber,
              }}
            >
              {it.status === "failed" ? (
                "⚠"
