...
export class AdminService {
  ...
  setNodeTrustLevel(id: string, trustLevel: NodeTrustLevel) {
    return this.prisma.computeNode.update({ where: { id }, data: { trustLevel } });
  }
  ...
  async dashboard() {
    ...
    const [
      ...
      this.prisma.job.count({ where: { status: 'COMPLETED', completedAt: { gte: startOfDay } } }),
    ...
    const creditAgg = await this.prisma.creditLedger.aggregate({ _sum: { amount: true } });
    const creditsNet = creditAgg._sum.amount ?? 0;

    return {
      users: { total: totalUsers, active: activeUsers },
      nodes: { active: activeNodes, online: onlineNodes },
      jobs: { pending: jobsPending, running: jobsRunning, completedToday: jobsCompletedToday, failed: failedJobs },
