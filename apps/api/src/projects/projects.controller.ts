...
export class ProjectsService {
  ...
  async pickFolder(initial?: string): Promise<{ path: string | null }> {
    ...
    const seed =
      initial && existsSync(initial)
        ? initial
        : home
          ? join(home, "Desktop")
          : "";
    const seedLine = seed
      ? `$f.SelectedPath = '${seed.replace(/'/g, "''")}'; `
      : "";
    const ps =
  ...
  create(user: AuthUser, name: string, path: string) {
    return this.prisma.project.create({
      data: { workspaceId: user.workspaceId, userId: user.sub, name, path },
    });
  }
  ...
  async previewHtml(
    ...
    dir: string,
    file?: string,
  ): Promise<{ files: string[]; file: string | null; content: string | null }> {
    const norm = (p: string) => p.replace(/\\/g, "/").replace(/\/+$/, "");
    const d = norm(dir || "");
    const projects = await this.list(user);
    const ok = projects.some((p) => {
      const pp = norm(p.path);
      return !!pp && (d === pp || d.startsWith(pp + "/"));
    });
    ...
    const scan = (rel: string, depth: number) => {
      ...
      for (const e of entries) {
        ...
        if (st.isDirectory()) {
          if (depth < 1 && !/node_modules|\.git/.test(e)) scan(r, depth + 1);
    ...
    if (!want || want.includes(".."))
      return { files: htmls, file: null, content: null };
    const full = pathResolve(join(dir, want));
    if (full !== pathResolve(dir) && !full.startsWith(pathResolve(dir) + sep))
      throw new ForbiddenException("path escapes the folder");
    let content: string | null = null;
    try {
      if (existsSync(full))
        content = readFileSync(full, "utf8").slice(0, 3_000_000);
    } catch {
      content = null;
    }
  ...
  async getRules(
    user: AuthUser,
    dir: string,
  ): Promise<{ file: string; exists: boolean; content: string }> {
      ...
      if (existsSync(p)) {
        try {
          return {
            file: name,
            exists: true,
            content: readFileSync(p, "utf8").slice(0, 12_000),
          };
        } catch {
          /* next */
        }
...
export class ProjectsController {
  ...
  preview(
    ...
    @Body() dto: { dir: string; file?: string },
  ) {
  ...
  setRules(
    @CurrentUser() user: AuthUser,
    @Body() dto: { dir: string; content: string },
  ) {
