...
export class VerificationService {
  ...
  private sanity(
    ...
    switch (job.type) {
      case "echo.v1": {
        ...
        return result.echo === input.text
          ? { ok: true }
          : { ok: false, reason: "echo mismatch" };
      }
      case "embedding.v1": {
        const v = result.vector as number[] | undefined;
        if (!Array.isArray(v) || v.length === 0)
          return { ok: false, reason: "bad vector" };
        if (v.some((x) => !Number.isFinite(x)))
          return { ok: false, reason: "NaN/Inf" };
        if (v.every((x) => x === 0))
          return { ok: false, reason: "zero vector" };
        return { ok: true };
      }
      case "image.v1": {
        ...
        if (!img.startsWith("iVBORw0KGgo"))
          return { ok: false, reason: "not a PNG" }; // base64 of the PNG magic
        return { ok: true };
      }
      default:
        return output?.success === true
          ? { ok: true }
          : { ok: false, reason: "no success flag" };
    }
  }
  ...
  private shouldSample(node: ComputeNode, reward: number): boolean {
    if (String(this.config.get("VERIFY_FORCE_SAMPLE") ?? "false") === "true")
      return true; // test hook
    if (
      node.lifecycleState === "SUSPENDED" ||
    ...
    const floor = Math.max(
      SAMPLE_FLOOR,
      SAMPLE_FLOOR + node.unverifiedJobCount / 20_000,
    );
  ...
  private deepCompare(
  ...
  ): { ok: boolean; detail: Prisma.InputJsonValue } {
    const result = output?.result ?? {};
    if (jobType === "echo.v1") {
    ...
    if (jobType === "embedding.v1") {
      ...
      if (!Array.isArray(out) || !Array.isArray(r))
        return {
          ok: false,
          detail: { method: "cosine", error: "missing vector" },
        };
  ...
  private async grant(
    ...
    if (optimistic) {
      ...
      await this.prisma.computeNode.update({
        where: { id: node.id },
        data: { outstandingOptimisticCredits: { increment: net } },
      });
      await this.prisma.job.update({
        where: { id: job.id },
        data: { grantedOptimistically: true },
      });
    }
  }
  ...
  private async finalize(
    ...
    await this.prisma.job.update({
      ...
      data: {
        ...
        nrnPayoutEligible: nrnEligible,
      },
    });
    await this.prisma.jobEvent.create({
      data: { jobId, type: "rewarded", data: { reward, score, nrnEligible } },
    });
  }
  ...
  private async slash(
    ...
    if (clawback > 0) {
      ...
      if (take > 0)
        await this.credits
          .spend(node.ownerUserId, take, "NODE_REWARD_CLAWBACK", {
            idempotencyKey: `clawback:${job.id}`,
          })
          .catch(() => undefined);
    }
    await this.prisma.computeNode.update({
    ...
    await this.prisma.ownerReputation
      .update({
        where: { userId: node.ownerUserId },
        data: {
          effectiveReputation: { decrement: 0.3 },
          deepFailCount: { increment: 1 },
        },
      })
      .catch(() => undefined);
    if (job.costCredits > 0) {
      await this.credits
        .grant(job.userId, job.costCredits, "USER_REFUND", `refund:${job.id}`)
        .catch(() => undefined);
    }
    await this.prisma.job.update({
      where: { id: job.id },
      data: {
        status: "FAILED",
        errorMessage: `verification failed: ${reason}`,
      },
    });
    await this.prisma.jobEvent.create({
      data: {
    ...
    this.logger.warn(
      `job ${job.id} SLASHED node ${node.id} (${reason}) — suspended + clawback ${clawback} + user refund`,
    );
    // Stake is forfeited (already spent at registration; never refunded for a fraudulent node).
    await this.retroAudit(node).catch((e) =>
      this.logger.error(`retro-audit failed: ${(e as Error).message}`),
    );
  }
  ...
  private async retroAudit(node: ComputeNode): Promise<void> {
    ...
    for (const j of jobs) {
      ...
      const deep = this.deepCompare(
        j.type,
        (j.outputJson ?? {}) as WorkerOutput,
        ref,
      );
      if (deep.ok) continue;
      frauds++;
      ...
      if (j.costCredits > 0)
        await this.credits
          .grant(j.userId, j.costCredits, "USER_REFUND", `refund:${j.id}`)
          .catch(() => undefined);
      await this.prisma.job
        .update({
          ...
          data: {
            status: "FAILED",
            errorMessage: "retro-audit: re-exec mismatch",
          },
        })
        .catch(() => undefined);
      await this.prisma.jobVerification
        .upsert({
          ...
          update: {
            ...
            sampled: true,
            resolvedAt: new Date(),
            detail: { retroAudit: true, ...(deep.detail as object) },
          },
          create: {
            jobId: j.id,
            jobType: j.type,
    ...
    if (frauds > 0)
      this.logger.warn(
        `retro-audit node ${node.id}: ${frauds} fraudulent prior job(s), ${clawed} credits clawed back`,
      );
  }
  ...
  async handleCompleted(jobId: string, output: WorkerOutput): Promise<void> {
    ...
    if (!s.ok) {
      await this.record(jobId, job.type, false, "SANITY_ONLY", "FAIL", 0, {
        reason: s.reason,
      });
    ...
    if (!sampled) {
      ...
      await this.prisma.computeNode.update({
        where: { id: node.id },
        data: {
          unverifiedJobCount: { increment: 1 },
    ...
    if (ref === null) {
      ...
      await this.record(
        ...
        "PENDING",
        0.5,
        { note: "trusted executor unavailable" },
        true,
      );
      await this.prisma.job.update({
        where: { id: jobId },
    ...
    const graduate =
      node.lifecycleState === "PROBATION" &&
      node.verifiedJobCount + 1 >= PROBATION_DEEP_PASSES;
    await this.prisma.job.update({
      where: { id: jobId },
      data: {
        status: "VERIFIED",
    ...
    if (graduate && node.stakeCredits > 0 && !node.stakeRefunded) {
      ...
      await this.prisma.computeNode
        .update({ where: { id: node.id }, data: { stakeRefunded: true } })
        .catch(() => undefined);
    }
