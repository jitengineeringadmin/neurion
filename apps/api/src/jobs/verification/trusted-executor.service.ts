...
export class TrustedExecutorService {
  ...
  private embedder: ((text: string, opts: unknown) => Promise<{ data: Float32Array }>) | null = null;
  ...
  available(jobType: string): boolean {
    ...
    if (jobType === 'embedding.v1') return String(this.config.get('VERIFY_EMBEDDING_ENABLED') ?? 'false') === 'true';
    return false;
  }

  /** Trusted reference output, or null if this deployment can't verify the type. */
  async reference(jobType: string, inputJson: unknown): Promise<Record<string, unknown> | null> {
    ...
    if (jobType === 'embedding.v1' && this.available(jobType)) {
      try {
        if (!this.embedder) {
          ...
          this.embedder = await tf.pipeline('feature-extraction', 'Xenova/all-MiniLM-L6-v2');
        }
        const out = await this.embedder(String(input.text ?? ''), { pooling: 'mean', normalize: true });
        return { vector: Array.from(out.data) };
      } catch (e) {
        this.logger.error(`embedding reference executor unavailable: ${(e as Error).message}`);
        return null;
      }
    }
    return null;
