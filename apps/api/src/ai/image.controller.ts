...
const CURATED: ModelDef[] = [
  ...
  },
  {
    id: "dreamshaper",
    label: "DreamShaper",
    desc: "Nicer photos, still fast",
    file: "dreamshaper.safetensors",
    sizeMb: 2000,
    recommended: true,
  ...
  {
    id: "dreamshaper-xl",
    label: "DreamShaper XL · high quality (heavy)",
    desc: "Best quality, needs a strong GPU",
    file: "dreamshaper-xl.safetensors",
    sizeMb: 6500,
...
export class ImageController {
  ...
  private binPath(dir: string) {
    return path.join(dir, "bin", CLI);
  }
  private binReady(dir: string) {
    return existsSync(this.binPath(dir));
  }
  private modelPath(dir: string, file: string) {
    return path.join(dir, "models", file);
  }
  private activePath(dir: string) {
  ...
  private modelOk(p: string, minMb = 1): boolean {
    try {
      return existsSync(p) && statSync(p).size >= minMb * 1_000_000;
    } catch {
      return false;
    }
  }
  ...
  status() {
    ...
    if (this.binReady(dir) && a)
      return {
        ...
        model: { label: a.label, id: a.id ?? "custom" },
      };
  ...
  models() {
    ...
    return {
      ...
      models: CURATED.map((m) => ({
        ...
        installed: dir
          ? this.modelOk(this.modelPath(dir, m.file), m.sizeMb * 0.9)
          : false,
      })),
    };
  }
  ...
  async selectModel(
    ...
    if (!dir) {
      send("error", { message: "image engine not available" });
      return void res.end();
    }
    if (!m) {
      send("error", { message: "unknown model" });
      return void res.end();
    }
    if (setup.installing) {
      send("error", { message: "setup already in progress" });
      return void res.end();
    }

    setup = { installing: true, percent: 0, stage: "engine", modelId: m.id };
    try {
      await this.ensureBin(dir, (pc) => {
        setup.percent = Math.round(pc * 0.05);
        setup.stage = "engine";
      ...
      if (!this.modelOk(mp, m.sizeMb * 0.9)) {
        ...
        await this.download(m.url, mp, (pc) => {
          setup.percent = 5 + Math.round(pc * 0.95);
          setup.stage = "model";
  ...
  async customModel(@Body() dto: { path?: string; label?: string }) {
    const dir = this.dir();
    if (!dir)
      return { ok: false as const, error: "image engine not available" };
    const p = (dto.path ?? "").trim();
    if (!p || !existsSync(p) || !NAME_RE.test(path.basename(p)))
      return {
        ok: false as const,
        error: "pick a .safetensors, .gguf or .ckpt file",
      };
    if (!this.binReady(dir)) {
      try {
        await this.ensureBin(dir, () => undefined);
      } catch (e) {
    ...
    const a: Active = {
      source: "custom",
      path: p,
      label: dto.label || path.basename(p),
      sampler: "euler_a",
      cfg: 7,
      steps: 20,
    };
  ...
  generate(@Body() dto: GenDto) {
    ...
    if (generating)
      return {
        ok: false as const,
        error: "another image is generating — one at a time",
      };
    ...
    const seed =
      Number.isFinite(Number(dto.seed)) && Number(dto.seed) >= 0
        ? Math.round(Number(dto.seed))
    ...
    const args = [
      ...
      "-p",
      prompt,
      ...(dto.negative && dto.negative.trim()
        ? ["-n", dto.negative.trim()]
        : []),
      "--cfg-scale",
      String(a.cfg),
    ...
    const finish = (status: string, error = "") => {
      ...
      imgRunningId = null;
      imgProc = null;
      if (imgCancelled.has(id)) {
        imgCancelled.delete(id);
        return;
      } // stopped — leave no meta
      try {
        writeFileSync(
          path.join(gdir, `${id}.json`),
          JSON.stringify({ ...meta, status, error }),
        );
      } catch {
        /* ignore */
      }
    };
    const cp = spawn(this.binPath(dir), args, {
      cwd: path.join(dir, "bin"),
      windowsHide: true,
    });
    imgProc = cp;
    let err = "";
    ...
    cp.on("error", (e) => {
      clearTimeout(to);
      finish("failed", (e as Error).message);
    });
    cp.on("close", (code) => {
      clearTimeout(to);
  ...
  gallery() {
    ...
    const items = metas.map((m) => {
      ...
      return {
        ...
        error: m.error,
        model: m.model,
        hasImage: m.status === "done" && existsSync(png),
      };
    });
    return { items };
  }
  ...
  file(@Param("id") id: string, @Res() res: Response) {
    const dir = this.dir();
    const png =
      dir && /^[a-f0-9-]{10,}$/i.test(id)
        ? path.join(this.galleryDir(dir), `${id}.png`)
        : "";
  ...
  private async ensureBin(
    ...
    if (!zipName)
      throw new Error(`image engine not supported on ${process.platform}`);
  ...
  private dim(v: unknown, def: number): number {
    return Math.round(this.clampInt(v, 256, 1024, def) / 64) * 64;
  }
}
