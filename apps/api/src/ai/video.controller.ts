...
const AI_FILES: { file: string; url: string; sizeMb: number }[] = [
  {
    file: "wan2.1-t2v-1.3b.safetensors",
...
const AI_FPS = 16;
const RUNNING_STALE_MS = 3 * 3600 * 1000 + 15 * 60 * 1000;
...
export class VideoController {
  ...
  private sdBin(dir: string) {
    return path.join(dir, "bin", CLI);
  }
  private galleryDir(dir: string) {
    return path.join(dir, "gallery");
  }
  private ffLocal(dir: string) {
    return path.join(dir, "ffbin", FF);
  }

  private active(dir: string): Active | null {
    try {
      const a = JSON.parse(
        readFileSync(path.join(dir, "active.json"), "utf8"),
      ) as Active;
  ...
  private aiDir(dir: string) {
    return path.join(dir, "video-models");
  }
  private aiPath(dir: string, file: string) {
    return path.join(this.aiDir(dir), file);
  }
  private aiInstalled(dir: string): boolean {
    return AI_FILES.every((f) => {
      try {
        return (
          statSync(this.aiPath(dir, f.file)).size >= f.sizeMb * 1_000_000 * 0.9
        );
      } catch {
        return false;
      }
  ...
  private requirements(dir: string) {
    ...
    try {
      ...
      freeDiskGb = Math.round((s.bavail * s.bsize) / 1_073_741_824);
    } catch {
      /* unknown */
    }
    return {
      ...
      diskOk: freeDiskGb < 0 || freeDiskGb >= MIN_DISK_GB,
    };
  }
  ...
  models() {
    ...
    return {
      ...
      files: AI_FILES.map((f) => ({
        file: f.file,
        sizeMb: f.sizeMb,
        installed: (() => {
          try {
            return (
              statSync(this.aiPath(dir, f.file)).size >=
              f.sizeMb * 1_000_000 * 0.9
            );
          } catch {
            return false;
          }
  ...
  async setupAi(@Res() res: Response) {
    ...
    try {
      ...
      for (const f of AI_FILES) {
        ...
        const ok = (() => {
          try {
            return statSync(out).size >= f.sizeMb * 1_000_000 * 0.9;
          } catch {
            return false;
          }
        })();
        if (!ok) {
          send("progress", {
            stage: f.file,
            percent: Math.round((doneMb / totalMb) * 100),
          });
          await this.download(f.url, out, (pct) =>
            send("progress", {
              stage: f.file,
              percent: Math.round(
                ((doneMb + (f.sizeMb * pct) / 100) / totalMb) * 100,
              ),
            }),
          );
        }
  ...
  private ffmpeg(dir: string): string | null {
    ...
    try {
      const r = spawnSync(FF.replace(".exe", ""), ["-version"], {
        windowsHide: true,
        timeout: 4000,
      });
  ...
  private saveMeta(galleryDir: string, meta: Record<string, unknown>): void {
    ...
    writeFileSync(path.join(galleryDir, `${id}.json`), JSON.stringify(meta));
  }

  private failRunningMeta(
    galleryDir: string,
    ...
    delete meta.etaS;
    meta.error = error;
    this.saveMeta(galleryDir, meta);
    const id = typeof meta.id === "string" ? meta.id : "";
    if (id)
      rmSync(path.join(galleryDir, `${id}-frames`), {
        recursive: true,
        force: true,
      });
  }

  private recoverRunningGallery(dir: string): void {
    const galleryDir = this.galleryDir(dir);
    if (!existsSync(galleryDir)) return;
    const now = Date.now();
    for (const file of readdirSync(galleryDir).filter((name) =>
      name.endsWith(".json"),
    )) {
      let meta: Record<string, unknown>;
      try {
        meta = JSON.parse(
          readFileSync(path.join(galleryDir, file), "utf8"),
        ) as Record<string, unknown>;
      } catch {
        continue;
      }
      if (meta.kind !== "video" || meta.status !== "running") continue;
      const id = typeof meta.id === "string" ? meta.id : "";
      if (id && id === runningId && busy) {
        const lastUpdate = Number(meta.updatedAt ?? meta.ts) || now;
        if (now - lastUpdate <= RUNNING_STALE_MS) continue;
        try {
          activeProc?.kill();
        } catch {
          /* already gone */
        ...
        runningId = null;
        cancelled.delete(id);
        this.failRunningMeta(
          galleryDir,
          meta,
          "Video generation timed out: no progress was saved for more than 3 hours.",
        );
        continue;
      }
      this.failRunningMeta(
        galleryDir,
        meta,
        "Interrupted video generation recovered after app restart or stopped worker.",
      );
    }
    for (const entry of readdirSync(galleryDir).filter((name) =>
      name.endsWith("-frames"),
    )) {
      const framesDir = path.join(galleryDir, entry);
      try {
        if (!statSync(framesDir).isDirectory()) continue;
      } catch {
        continue;
      }
      const id = entry.slice(0, -"-frames".length);
      if (id === runningId && busy) continue;
      let status: unknown = null;
      try {
        status = (
          JSON.parse(
            readFileSync(path.join(galleryDir, `${id}.json`), "utf8"),
          ) as { status?: unknown }
        ).status;
      } catch {
        /* orphan */
  ...
  status() {
    ...
    if (!existsSync(this.sdBin(dir)) || !this.active(dir))
  ...
  generate(@Body() dto: VideoDto) {
    ...
    const dir = this.dir();
    if (!dir || !existsSync(this.sdBin(dir)))
      return {
        ok: false as const,
        error: "image engine not ready — set up a model first",
      };
    const a = this.active(dir);
    if (dto.mode !== "ai" && !a)
      return { ok: false as const, error: "no image model selected" };
    ...
    if (busy)
      return {
        ok: false as const,
        error: "another video is generating — one at a time",
      };

    const ai = dto.mode === "ai";
    if (ai) {
      if (!this.aiInstalled(dir))
        return { ok: false as const, error: "AI video model not installed" };
      const req = this.requirements(dir);
      if (!req.ramOk)
        return {
          ok: false as const,
          error: `minimum requirements not met: ${req.ramGb}GB RAM (need ${MIN_RAM_GB}GB+)`,
        };
    }
    ...
    const meta: Record<string, unknown> = {
      ...
      ai,
      prompt,
      model: ai ? "Wan 2.1" : (a?.label ?? ""),
      status: "running",
      progress: ai ? "ai" : "0/" + FRAMES,
      ts: Date.now(),
      updatedAt: Date.now(),
      error: "",
    };
  ...
  private async muxAudio(
    ...
    try {
      ...
      if (inputs.length === 2) {
        // music (ducked) + voice
        filter =
          "[1:a]volume=0.25,afade=t=in:d=0.8[m];[2:a]volume=1.0[v];[m][v]amix=inputs=2:duration=longest:dropout_transition=0[a]";
        maps = ["-map", "0:v", "-map", "[a]"];
      } else {
  ...
  private async renderAi(
    ...
    try {
      ...
      const args = [
        ...
        "vid_gen",
        "--diffusion-model",
        this.aiPath(dir, "wan2.1-t2v-1.3b.safetensors"),
        "--vae",
        this.aiPath(dir, "wan_2.1_vae.safetensors"),
        "--t5xxl",
        this.aiPath(dir, "umt5-xxl-q5.gguf"),
        "--offload-to-cpu",
        "--backend",
        "te=cpu,vae=cpu",
        "-p",
      ...
      if (avi) {
        ...
        frames.forEach((f, i) =>
          renameSync(
            path.join(tmp, f),
            path.join(tmp, `fr${String(i).padStart(4, "0")}.png`),
          ),
        );
        await this.run(
          ff,
      ...
      if (!existsSync(outMp4) || statSync(outMp4).size < 10_000)
      ...
      meta.status = "failed";
      meta.error = String((e as Error).message).slice(-200);
      save();
    } finally {
  ...
  private async render(
    ...
    try {
      ...
      for (let i = 0; i < FRAMES; i++) {
        const z =
          i % 2 === 0 ? `min(1+0.0028*on,1.25)` : `max(1.25-0.0028*on,1.0)`;
        zooms.push(
          `[${i}:v]scale=2048:2048:flags=lanczos,zoompan=z='${z}':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${d}:s=720x720:fps=${fps},setsar=1[v${i}]`,
        );
      }
      const xf: string[] = [];
      let prev = "v0";
      for (let i = 1; i < FRAMES; i++) {
        ...
        xf.push(
          `[${prev}][v${i}]xfade=transition=fade:duration=${FADE_S}:offset=${offset.toFixed(2)}[${out}]`,
        );
        prev = out;
      }
      const outMp4 = path.join(gdir, `${id}.mp4`);
  ...
  private run(
    ...
    return new Promise((resolve, reject) => {
      ...
      const done = () => {
        if (activeProc === cp) activeProc = null;
      };
      const to = setTimeout(() => {
        cp.kill();
      ...
      cp.on("close", (code) => {
        ...
        code === 0
          ? resolve()
          : reject(new Error(err.slice(-200) || `exited ${code}`));
      });
    });
  }
  ...
  gallery() {
    ...
    const items = readdirSync(gdir)
      ...
      .map((f) => {
        try {
          return JSON.parse(readFileSync(path.join(gdir, f), "utf8")) as Record<
            string,
            unknown
          >;
        } catch {
