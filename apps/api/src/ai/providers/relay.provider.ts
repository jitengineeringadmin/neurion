  ...
  async *streamChat(messages: ChatMsg[], model: string, signal?: AbortSignal): AsyncIterable<string> {
    ...
    for (;;) {
      ...
      for (const frame of frames) {
        ...
        if (!data) continue;
        if (event === 'token') {
          try { yield (JSON.parse(data) as { t: string }).t; } catch { /* skip */ }
        } else if (event === 'node') {
          try { this.lastNodeId = (JSON.parse(data) as { nodeId: string }).nodeId; } catch { /* skip */ }
        } else if (event === 'error') {
          let msg = 'relay error';
          try { msg = (JSON.parse(data) as { message: string }).message; } catch { /* keep default */ }
          throw new Error(msg);
        }
