...
const VOICE_BASE = 'https://huggingface.co/rhasspy/piper-voices/resolve/main/it/it_IT/paola/medium';
...
export class AudioService {
  ...
  dir(): string | null {
    const base = this.config.get<string>('NEURION_IMAGE_DIR') || process.env.NEURION_IMAGE_DIR || null;
    return base ? path.join(base, 'audio') : null;
  }
  musicPath(mood: Mood) { const d = this.dir(); return d ? path.join(d, 'music', `${mood}.mp3`) : ''; }
  private piperExe() { const d = this.dir(); return d ? path.join(d, 'piper', 'piper', 'piper.exe') : ''; }
  private voicePath() { const d = this.dir(); return d ? path.join(d, 'piper', VOICE) : ''; }

  musicReady(): boolean { return MOODS.every((m) => { try { return statSync(this.musicPath(m)).size > 50_000; } catch { return false; } }); }
  ttsReady(): boolean { return existsSync(this.piperExe()) && existsSync(this.voicePath()); }
  genReady(): boolean { const d = this.dir(); return !!d && existsSync(path.join(d, 'gen.ready')); }

  status() {
    const d = this.dir();
    if (!d) return { music: 'unavailable', tts: 'unavailable', gen: 'unavailable' };
    return {
      music: this.musicReady() ? 'ready' : 'needs_setup',
      tts: this.ttsReady() ? 'ready' : process.platform === 'win32' ? 'needs_setup' : 'unsupported',
  ...
  async setupMusic(onPct: (p: number) => void): Promise<void> {
    ...
    for (let i = 0; i < MOODS.length; i++) {
      ...
      if (!existsSync(out) || statSync(out).size < 50_000) {
        await this.download(`${MUSIC_BASE}/${m}.mp3`, out, (pct) => onPct(Math.round(((i + pct / 100) / MOODS.length) * 100)));
      }
  ...
  async setupTts(onPct: (p: number) => void): Promise<void> {
    const d = this.dir();
    if (!d) throw new Error('audio unavailable in this deployment');
    if (process.platform !== 'win32') throw new Error('install piper manually on this OS');
    const pdir = path.join(d, 'piper');
    mkdirSync(pdir, { recursive: true });
    if (!existsSync(this.piperExe())) {
      const zipPath = path.join(pdir, 'piper.zip');
      await this.download(PIPER_ZIP, zipPath, (p) => onPct(Math.round(p * 0.2)));
    ...
    if (!existsSync(this.voicePath())) {
      await this.download(`${VOICE_BASE}/${VOICE}`, this.voicePath(), (p) => onPct(20 + Math.round(p * 0.75)));
      await this.download(`${VOICE_BASE}/${VOICE}.json`, `${this.voicePath()}.json`, () => onPct(97));
    }
    onPct(100);
  }

  /** Load MusicGen once (first call downloads the ONNX model into our cache dir). */
  private async loadGen(onPct?: (p: number) => void): Promise<(prompt: string) => Promise<{ audio: Float32Array; sampling_rate: number }>> {
    const d = this.dir();
    if (!d) throw new Error('audio unavailable in this deployment');
    if (!this.genPipeline) {
      ...
      let last = 0;
      this.genPipeline = await pipeline('text-to-audio', 'Xenova/musicgen-small', {
        progress_callback: (p: { progress?: number }) => {
          const pct = Math.round(p.progress ?? 0);
          if (onPct && pct !== last) { last = pct; onPct(pct); }
        },
      } as never);
      writeFileSync(path.join(d, 'gen.ready'), '1');
    }
    const gen = this.genPipeline as (t: string, o: Record<string, unknown>) => Promise<{ audio: Float32Array; sampling_rate: number }>;
    return (prompt: string) => gen(prompt, { max_new_tokens: 750, do_sample: true, guidance_scale: 3 });
  }

  async setupGen(onPct: (p: number) => void): Promise<void> { await this.loadGen(onPct); }
  ...
  async tts(text: string, outWav: string): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      const cp = spawn(this.piperExe(), ['-m', this.voicePath(), '-c', `${this.voicePath()}.json`, '-f', outWav], { windowsHide: true });
      let err = '';
      cp.stderr.on('data', (c) => (err += c.toString()));
      const to = setTimeout(() => { cp.kill(); reject(new Error('tts timed out')); }, 120000);
      cp.on('error', (e) => { clearTimeout(to); reject(e); });
      cp.on('close', (code) => { clearTimeout(to); code === 0 && existsSync(outWav) ? resolve() : reject(new Error(err.slice(-160) || `piper exited ${code}`)); });
      cp.stdin.write(text);
      cp.stdin.end();
    });
  }
  ...
  private wav(f32: Float32Array, rate: number): Buffer {
    ...
    for (let i = 0; i < f32.length; i++) pcm[i] = Math.max(-32768, Math.min(32767, Math.round((f32[i] ?? 0) * 32767)));
