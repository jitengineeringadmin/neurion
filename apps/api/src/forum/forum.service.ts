...
const isMod = (role: string): boolean =>
...
export class ForumService {
    ...
    for (const th of threads) {
      ...
      if (!s.last)
        s.last = {
          ...
          at: th.lastActivityAt,
        }; // ordered desc -> first is newest
  ...
  async createThread(
    userId: string,
    data: { sectionId: string; title: string; body: string },
  ) {
  ...
  async deletePost(user: AuthUser, postId: string) {
    ...
    if (p.authorId !== user.sub && !isMod(user.role))
  ...
  async deleteThread(user: AuthUser, threadId: string) {
    ...
    if (!t) throw new NotFoundException("thread not found");
    if (t.authorId !== user.sub && !isMod(user.role))
      throw new ForbiddenException("not allowed");
