...
function vaultStub(mode: "ok" | "throw-on-send" | "throw-on-wait") {
  return {
    getFunction: () => async () => {
      if (mode === "throw-on-send")
        throw new Error("insufficient funds for gas");
      return {
        hash: "0xdeadbeef",
        wait: async () => {
          if (mode === "throw-on-wait")
...
async function freshUser(
  credits: number,
  kyc: "KYC_APPROVED" | "KYC_REQUIRED" = "KYC_REQUIRED",
) {
  const ws = await prisma.workspace.findFirst();
  const workspaceId =
    ws?.id ??
    (await prisma.workspace.create({ data: { name: "test", slug: "test" } }))
      .id;
...
void (async () => {
  ...
  await test("a payout that never reaches the chain refunds the GROSS, fee included", async () => {
    ...
    const svc = new TokenPayoutService(
      P,
      credits,
      tokenConfig("throw-on-send"),
      auditStub,
      emissionStub,
    );
    ...
    assert.equal(
      afterRequest,
      300,
      `expected 300 after spending 200, got ${afterRequest}`,
    );
    ...
    assert.equal(
      afterFailure,
      500,
      `expected the full 500 back, got ${afterFailure}`,
    );

    const p = await prisma.tokenPayout.findFirstOrThrow({
      where: { userId: u.id },
    });
  ...
  await test("SECURITY: a submitted payout is NOT refunded when confirmation is not observed", async () => {
    ...
    assert.equal(
      p.status,
      "SUBMITTED",
      "must stay SUBMITTED for reconciliation, not FAILED",
    );
    assert.ok(
      p.txHash,
      "the transaction hash must be kept so the send can be reconciled",
    );
  ...
  await test("SECURITY: splitting a cash-out no longer stays under the KYC threshold", async () => {
    ...
    for (let i = 0; i < 3; i++)
      await svc.requestPayout({ sub: u.id } as never, 300);
  ...
  await test("a failed payout does not count towards the KYC window", async () => {
    const u = await freshUser(5000);
    const bad = new TokenPayoutService(
      P,
      credits,
      tokenConfig("throw-on-send"),
      auditStub,
    ...
    const ok = new TokenPayoutService(
      P,
      credits,
      tokenConfig("ok"),
      auditStub,
    ...
    const n = await prisma.tokenPayout.count({
      where: { userId: u.id, status: { not: "FAILED" } },
    });
    assert.equal(n, 1);
  });
