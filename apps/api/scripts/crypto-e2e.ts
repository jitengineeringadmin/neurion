...
async function main(): Promise<void> {
  ...
  const linked = await req(
    "/wallet/verify",
    "POST",
    { address: wallet.address, nonce: nonceRes.nonce, signature },
    userTok,
  );
  ...
  if (TOKEN_ADDRESS) {
    ...
    const bal = (await token.getFunction("balanceOf")(
      wallet.address,
    )) as bigint;
    console.log(
      `on-chain NRN balance of wallet: ${ethers.formatEther(bal)} NRN (expect 0.5)`,
    );
  }
}
