...
function execute(
  ...
  return {
    success: true,
    result,
    metrics: { startedAt, completedAt: new Date().toISOString() },
  };
}
    ...
    for (;;) {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (delta)
            send({ type: "realtime.chat.token", requestId, text: delta });
        } catch {
          /* ignore */
        }
      }
...
async function main(): Promise<void> {
  ...
  const reg = await post(
    "/nodes/register",
    { name: "fake-node-ts", supportedJobTypes: ["echo.v1", "embedding.v1"] },
    accessToken,
  );
    ...
    setInterval(
      () =>
        send({
          type: "node.heartbeat",
          nodeId: reg.nodeId,
          metrics: { cpuLoad: 12.3, ramUsedMb: 4096 },
        }),
      10_000,
    );
  });
  ...
  ws.on("message", (raw) => {
    ...
    if (msg.type === "hello.ok") {
      ...
      send({ type: "job.accepted", jobId: job.id, attempt: job.attempt });
      send({ type: "job.started", jobId: job.id, attempt: job.attempt });
